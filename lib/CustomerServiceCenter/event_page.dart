...
import 'firestore_service.dart'; // 이벤트 추가 페이지를 가져옵니다.
...
class _EventPageState extends State<EventPage> {
  ...
  Future<void> _loadSessionData() async {
    ...
    if (email != null) {
      ...
      bool adminStatus = await FirestoreService()
          .isAdminByEmail(email); // FirestoreService를 사용하여 관리자 여부를 확인합니다.
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('이벤트'), // 앱바 제목을 설정합니다.
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('events').snapshots(),
        // Firestore의 이벤트 컬렉션을 스트림으로 가져옵니다.
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            // 에러가 발생한 경우
            return Center(
                child: Text('Error: ${snapshot.error}')); // 에러 메시지를 표시합니다.
          } else {
            // 데이터가 성공적으로 로드된 경우
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return EventCard(
                    event: event, isAdmin: isAdmin); // 이벤트 카드를 반환합니다.
              },
            );
          }
        },
          ...
          ? FloatingActionButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          AddEventPage()), // 이벤트 추가 페이지로 이동합니다.
                );
              },
